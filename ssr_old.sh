...
set_port_pass(){
	...
	stty erase '^H' && read -p "(${Word_default}: fei5seven):" sspwd
	[[ -z "${sspwd}" ]] && sspwd="fei5seven"
...
JQ_install(){
	...
	if [[ -z ${JQ_ver} ]]; then
		...
		wget --no-check-certificate -N "https://raw.githubusercontent.com/fei5seven/shell/master/other/jq-1.5.tar.gz"
...
EOF
	...
	if [[ ! -z "${PID}" ]]; then
		...
		echo -e "https://fei5seven/ss-jc42/"
		echo
		echo "############################################################"
	else
...
BanBTPTSPAM(){
	wget -4qO- raw.githubusercontent.com/fei5seven/shell/master/Get_Out_Spam.sh | bash
}
InstallBBR(){
