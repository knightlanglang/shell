...
#	Version: 1.1.13
#	
#	Blog: https://fei5seven/brook-jc3/
#=================================================
...
Service_brook(){
	if [[ ${release} = "centos" ]]; then
		if ! wget --no-check-certificate "https://raw.githubusercontent.com/fei5seven/shell/master/service/brook_centos" -O /etc/init.d/brook; then
		...
		if ! wget --no-check-certificate "https://raw.githubusercontent.com/fei5seven/shell/master/service/brook_debian" -O /etc/init.d/brook; then
			echo -e "${Error} Brook服务 管理脚本下载失败 !" && rm -rf "${file}" && exit 1
		fi
		chmod +x "/etc/init.d/brook"
...
Set_passwd(){
	...
	read -e -p "(默认: fei5seven):" bk_passwd
	[[ -z "${bk_passwd}" ]] && bk_passwd="fei5seven"
	echo && echo "========================"
...
View_brook(){
	...
	echo
	echo -e "${Tip} Brook链接 仅适用于Windows系统的 Brook Tools客户端（https://fei5seven/dbrj-7/）。"
	echo
}
brook_link(){
...
Update_Shell(){
	sh_new_ver=$(wget --no-check-certificate -qO- -t1 -T3 "https://raw.githubusercontent.com/fei5seven/shell/master/brook.sh"|grep 'sh_ver="'|awk -F "=" '{print $NF}'|sed 's/\"//g'|head -1) && sh_new_type="github"
	[[ -z ${sh_new_ver} ]] && echo -e "${Error} 无法链接到 Github !" && exit 0
	if [[ -e "/etc/init.d/brook" ]]; then
		rm -rf /etc/init.d/brook
		Service_brook
	fi
	wget -N --no-check-certificate "https://raw.githubusercontent.com/fei5seven/shell/master/brook.sh" && chmod +x brook.sh
