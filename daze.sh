...
Service(){
	if [[ ${release} = "centos" ]]; then
		if ! wget --no-check-certificate https://raw.githubusercontent.com/fei5seven/shell/master/service/daze_centos -O /etc/init.d/daze; then
		...
		if ! wget --no-check-certificate https://raw.githubusercontent.com/fei5seven/shell/master/service/daze_debian -O /etc/init.d/daze; then
			echo -e "${Error} DAZE 服务管理脚本下载失败 !" && rm -rf "${Folder}" && exit 1
		fi
		chmod +x /etc/init.d/daze
...
Set_password(){
	echo "请输入 DAZE 密码"
	read -e -p "(默认: fei5seven):" new_password
	[[ -z "${new_password}" ]] && new_password="fei5seven"
	echo && echo "========================"
...
View_daze(){
	...
	echo -e "${link}"
	echo -e "${Tip} 链接仅适用于Windows系统的 DAZE Tools 客户端（https://fei5seven/dbrj-17/）。"
	echo && echo "————————————————"
}
urlsafe_base64(){
...
Update_Shell(){
	sh_new_ver=$(wget --no-check-certificate -qO- -t1 -T3 "https://raw.githubusercontent.com/fei5seven/shell/master/daze.sh"|grep 'sh_ver="'|awk -F "=" '{print $NF}'|sed 's/\"//g'|head -1) && sh_new_type="github"
	[[ -z ${sh_new_ver} ]] && echo -e "${Error} 无法链接到 Github !" && exit 0
	if [[ -e "/etc/init.d/daze" ]]; then
		rm -rf /etc/init.d/daze
		Service
	fi
	wget -N --no-check-certificate "https://raw.githubusercontent.com/fei5seven/shell/master/daze.sh" && chmod +x daze.sh
