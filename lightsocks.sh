...
#	Version: 1.0.1
#	
#	Blog: https://fei5seven/lightsocks-jc1/
#=================================================
...
Service_lightsocks(){
	if [[ ${release} = "centos" ]]; then
		if ! wget --no-check-certificate "https://raw.githubusercontent.com/fei5seven/shell/master/service/lightsocks_centos" -O /etc/init.d/lightsocks; then
		...
		if ! wget --no-check-certificate "https://raw.githubusercontent.com/fei5seven/shell/master/service/lightsocks_debian" -O /etc/init.d/lightsocks; then
			echo -e "${Error} Lightsocks服务 管理脚本下载失败 !" && rm -rf "${file}" && exit 1
		fi
		chmod +x "/etc/init.d/lightsocks"
...
View_lightsocks(){
	...
	echo
	echo -e "${Tip} Lightsocks链接 仅适用于Windows系统的 Lightsocks Tools客户端[https://fei5seven/dbrj-12/]。"
	echo
}
urlsafe_base64(){
...
Update_Shell(){
	sh_new_ver=$(wget --no-check-certificate -qO- -t1 -T3 "https://raw.githubusercontent.com/fei5seven/shell/master/lightsocks.sh"|grep 'sh_ver="'|awk -F "=" '{print $NF}'|sed 's/\"//g'|head -1) && sh_new_type="github"
	[[ -z ${sh_new_ver} ]] && echo -e "${Error} 无法链接到 Github !" && exit 0
	if [[ -e "/etc/init.d/lightsocks" ]]; then
		rm -rf /etc/init.d/lightsocks
		Service_lightsocks
	fi
	wget -N --no-check-certificate "https://raw.githubusercontent.com/fei5seven/shell/master/lightsocks.sh" && chmod +x lightsocks.sh
